...
if ( ! function_exists( 'tld_actt_review_notice' ) ) {
  // Add an admin notice.
  add_action('admin_notices', 'tld_actt_review_notice');
  /**
  ...
  *  @version 1.0.0
  */
  function tld_actt_review_notice() {
    // Define your Plugin name, review url, and donation url.
    $plugin_name = 'Awesome Click to Tweet';
    $review_url = 'https://wordpress.org/support/plugin/awesome-click-to-tweet/reviews/#new-post';
    ...
    $tld_actt_triggerdate = get_option( 'tld_actt_activation_date', false );
    $installed = ( ! empty( $tld_actt_triggerdate ) ? $tld_actt_triggerdate : '999999999999999' );
    ...
    if ( $installed <= $today && danp_is_super_admin_admin( $current_user = $current_user ) == true ) {
      ...
      if ( 'plugins.php' == $pagenow ) {
        ...
        $nag_admin_dismiss_url = 'plugins.php?tld_actt_review_dismiss=0';
        $user_meta             = get_user_meta( $user_id, 'tld_actt_review_dismiss' );
        ...
        if ( empty( $user_meta ) ) {
          ...
          <p class="review"><span class="dashicons dashicons-heart"></span><?php echo wp_kses( sprintf( __( 'Hey :) hope you\'re enjoying <strong>' . $plugin_name . '</strong>. Would you please consider <a href="%1$s" target="_blank">rating this plugin?</a> 5 stars would be nice :) I\'m making it better and need your support, thanks.', 'actt' ), esc_url( $review_url ) ), array( 'strong' => array(), 'a' => array( 'href' => array(), 'target' => array() ) ) ); ?><a href="<?php echo admin_url( $nag_admin_dismiss_url ); ?>" class="dismiss"><span class="dashicons dashicons-dismiss"></span></a>
          </div>
  ...
  if ( ! function_exists( 'tld_actt_ignore_review_notice' ) ) {
    // Function to force the Review Admin Notice to stay dismissed correctly.
    add_action('admin_init', 'tld_actt_ignore_review_notice');
    /**
    * Ignore review notice.
    *
    * @since  1.0.0
    */
    function tld_actt_ignore_review_notice() {

      if ( isset( $_GET[ 'tld_actt_review_dismiss' ] ) && '0' == $_GET[ 'tld_actt_review_dismiss' ] ) {
        // Get the global user.
        global $current_user;
        $user_id = $current_user->ID;
        add_user_meta( $user_id, 'tld_actt_review_dismiss', 'true', true );
      }
